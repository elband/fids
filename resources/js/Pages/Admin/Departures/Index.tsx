...
import { Edit, Trash, Plus, Monitor, RefreshCw, CalendarCheck, PlaneTakeoff, Database } from 'lucide-react';
...
export default function Index({ 
    ...
    // Auto-refresh logic
    useEffect(() => {
        // 1. Regular 5-minute refresh for general data freshness
        const interval = setInterval(() => {
            router.reload({ only: ['flights', 'today_name', 'today_date'] });
            setLastRefresh(new Date());
        }, 5 * 60 * 1000);

        // 2. Scheduled 1 AM refresh to handle date transitions and schedule updates
        const checkOneAM = setInterval(() => {
            ...
            const lastOneAM = sessionStorage.getItem('last_1am_refresh');

            // If it's exactly 1:00 AM in server timezone and we haven't refreshed yet today
            if (hourStr === '01' && minStr === '00' && lastOneAM !== todayStr) {
                sessionStorage.setItem('last_1am_refresh', todayStr);
                router.reload();
                setLastRefresh(new Date());
            }
    ...
    const handleDayFilterChange = (day: string) => {
        router.get(route(isDaily ? 'admin.daily-departures.index' : 'admin.departures.index'),
            { day: day },
            {}
        );
    };
    ...
    const openModal = (flight: any = null) => {
        if (flight) {
            ...
            setData({
                ...
                checkin_counter_ids: flight.checkin_counters?.map((c: any) => c.id) || [],
            });
        } else {
            setEditingId(null);
    ...
    const submit = (e: React.FormEvent) => {
        ...
        const routePrefix = isDaily ? 'admin.daily-departures' : 'admin.departures';
    ...
    const deleteFlight = async (id: number) => {
        ...
        const ok = await appConfirm({
            ...
            message: 'Apakah Anda yakin ingin menghapus jadwal keberangkatan ini?',
            confirmText: 'Hapus',
            cancelText: 'Batal',
        });
        if (!ok) return;
    ...
    const pullDataMaster = async () => {
        ...
        if (!ok) return;
        post(route('admin.daily-departures.pull'));
    };
        ...
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                    {isDaily ? 'Keberangkatan (Hari Ini)' : 'Master Keberangkatan'}
                </h2>
            }
        >
            <Head title={isDaily ? 'Keberangkatan (Hari Ini)' : 'Master Keberangkatan'} />

            <div className="py-8">
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8 space-y-6">
                    {/* HERO */}
                    <div className="relative overflow-hidden rounded-3xl shadow-xl border border-white/10 bg-gradient-to-br from-sky-600 via-blue-600 to-indigo-700 text-white">
                        <div aria-hidden className="pointer-events-none absolute -top-24 -right-24 w-96 h-96 rounded-full bg-sky-300/30 blur-3xl" />
                        <div aria-hidden className="pointer-events-none absolute -bottom-20 -left-16 w-80 h-80 rounded-full bg-indigo-300/30 blur-3xl" />
                        ...
                        <div className="relative grid grid-cols-1 lg:grid-cols-3 gap-6 p-6 lg:p-8">
                            <div className="lg:col-span-2 space-y-3">
                                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15 backdrop-blur ring-1 ring-white/20 text-[11px] font-bold uppercase tracking-[0.2em]">
                                    <PlaneTakeoff size={12} /> {isDaily ? 'Operasional' : 'Master Schedule'}
                                </div>
                                <h3 className="text-3xl lg:text-4xl font-black tracking-tight drop-shadow-md">
                                    {isDaily ? 'Keberangkatan Hari Ini' : 'Master Jadwal Keberangkatan'}
                                </h3>
                                {isDaily ? (
                                    <p className="text-sm lg:text-base text-white/85 max-w-xl">
                                        Jadwal penerbangan berangkat untuk tanggal <span className="font-bold">{formatTanggal(today_date)}</span>.
                                        Data sinkron otomatis tiap 5 menit dan refresh ulang pada pukul 01.00 WITA.
                                    </p>
                                ) : (
                                    <p className="text-sm lg:text-base text-white/85 max-w-xl">
                                        Database master jadwal penerbangan berangkat per hari operasi. Data ini menjadi sumber jadwal harian.
                                    </p>
                                )}
                                ...
                                <div className="flex flex-wrap items-center gap-2 pt-1">
                                    ...
                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/15 backdrop-blur ring-1 ring-white/20 text-xs font-semibold">
                                        <PlaneTakeoff size={12} /> {flights?.total ?? flights.data.length} jadwal
                                    </span>
                                ...
                                {!isDaily && (
                                    <div className="flex flex-wrap items-center gap-2 pt-2">
                                        ...
                                        <select
                                            ...
                                            className="text-xs rounded-lg border-white/20 bg-white/10 backdrop-blur text-white py-1.5 pl-3 pr-8 focus:ring-sky-300"
                                        >
                            ...
                            <div className="flex flex-col gap-2 justify-end">
                                <button
                                    onClick={() => openModal()}
                                    className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white text-sky-700 font-bold shadow-lg hover:shadow-2xl hover:-translate-y-0.5 transition active:translate-y-0"
                                >
                                    <Plus size={16} /> Tambah Data
                                </button>
                                <a
                                    href={route('display.departure')}
                                    target="_blank"
                                ...
                                {isDaily && (
                                    <button
                                        ...
                                        className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-sky-900/30 backdrop-blur ring-1 ring-white/20 text-white font-semibold hover:bg-sky-900/50 transition"
                                    >
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full shadow-xl p-6 max-h-[90vh] overflow-y-auto">
                        <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-gray-100">
                            {editingId ? 'Edit Keberangkatan' : 'Tambah Keberangkatan'}
                        </h3>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 max-h-60 overflow-y-auto p-2 border rounded-md dark:border-gray-700 dark:bg-gray-900">
                                        {checkinCounters.map((a: any) => (
                                            <label key={a.id} className="flex items-center space-x-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 p-1 rounded">
                                                <input 
                                                    type="checkbox" 
                                                    checked={data.checkin_counter_ids.includes(a.id)}
                                                    onChange={e => {
                                                        const checked = e.target.checked;
                                                        const newIds = checked 
                                                            ? [...data.checkin_counter_ids, a.id]
                                                            : data.checkin_counter_ids.filter(id => id !== a.id);
                                                        setData('checkin_counter_ids', newIds);
                                                    }}
                                                    className="rounded border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500"
                                                />
                                                <span>Counter {a.nomor_counter}</span>
                                            </label>
                                        ))}
                                    </div>
                                    <p className="text-[10px] text-gray-500 mt-1 italic">* Anda dapat memilih lebih dari satu counter</p>
                                </div>
                            </div>
